    ...
    grunt.registerMultiTask('angularFileLoader', 'Automatically sort and inject AngularJS app files depending on module definitions and usage', function () {
        ...
        function resolvePath(from, to) {
            if (grunt.util.kindOf(options.relative) === "string") {
				...
				grunt.log.debug("tet " + (path.relative(path.dirname(from), to)).replace(/\\/g, '/'));
        ...
        function inject(file) {
			...
			var EOLLEN = splitedFile[0].length - splitedFile[0].lastIndexOf("\n");
			var indentation = splitedFile[0].substr(splitedFile[0].length-EOLLEN, EOLLEN);
			grunt.log.debug("EOL "+(splitedFile[0].length - splitedFile[0].lastIndexOf("\n")));
			grunt.log.debug("sort0 "+splitedFile[0]);
			grunt.log.debug("indent |"+indentation+"|");
			...
			sortedScripts.forEach(function (script) {
				grunt.log.debug("sort "+script);
                splitedFile[1] += indentation + ((supportedExt[pattern]["recipe"]).replace('%', resolvePath(file, script)));
				grunt.log.debug("sort1 "+splitedFile[1]);
            });
            splitedFile[2] = indentation + supportedExt[pattern]["comment"]["start"] + options.endTag + supportedExt[pattern]["comment"]["end"];
			grunt.log.debug("sort2 "+splitedFile[2]);
			var splitedFile2 = [splitedFile[0], splitedFile[1], splitedFile[3]];
            try {
        ...
        function inject(file) {
            ...
            var splitedFile = (grunt.file.read(file)).split(supportedExt[pattern]["regex"]);
            var indentation = splitedFile[0].substr( splitedFile[0].search(/\S/) + 1 );
			grunt.log.debug("sort0 "+splitedFile[0]);
			grunt.log.debug("indent "+indentation);
            //splitedFile[1] = supportedExt[pattern]["comment"]["start"] + options.startTag + supportedExt[pattern]["comment"]["end"] + EOL;
            sortedScripts.forEach(function (script) {
            ...
            splitedFile[2] = indentation + supportedExt[pattern]["comment"]["start"] + options.endTag + supportedExt[pattern]["comment"]["end"];
			grunt.log.debug("sort2 "+splitedFile[2]);
			var splitedFile2 = [splitedFile[1], splitedFile[3]];
            try {
                grunt.file.write(file, splitedFile2.join(''));
                grunt.log.ok(sortedScripts.length + " insert into " + file);
            } catch (error) {
                throw grunt.log.error("Can't write in" + file + " error : " + error);
            }
            ...
            this.files.forEach(function (filegroup) {
                ...
                filegroup.src.filter(function (file) {
                    ...
                    // Test if it contain the pattern
						...
						grunt.log.error('injecting "' + file);
                        inject(file);
                    }
                });
            });
